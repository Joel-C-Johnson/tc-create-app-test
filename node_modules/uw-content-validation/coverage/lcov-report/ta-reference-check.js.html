...
<head>
    <title>Code coverage report for ta-reference-check.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="index.html">All files</a> ta-reference-check.js</h1>
        <div class='clearfix'>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import { cachedGetFile } from '../core/getApi';
...
&nbsp;
    function addNoticePartial(noticeObject) {
        ...
        <span class="missing-if-branch" title="if path not taken" >I</span>if (noticeObject.characterIndex) { //parameterAssert(typeof noticeObject.characterIndex === 'number', `cTAref addNoticePartial: 'characterIndex' parameter should be a number not a '${typeof noticeObject.characterIndex}': ${noticeObject.characterIndex}`);
        }
        // //parameterAssert(excerpt !== undefined, "cTAref addNoticePartial: 'excerpt' parameter should be defined");
        <span class="missing-if-branch" title="else path not taken" >E</span>if (noticeObject.excerpt) { //parameterAssert(typeof noticeObject.excerpt === 'string', `cTAref addNoticePartial: 'excerpt' parameter should be a string not a '${typeof noticeObject.excerpt}': ${noticeObject.excerpt}`);
        }
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!taRepoUsername) <span class="cstat-no" title="statement not covered" >taRepoUsername = 'Door43-Catalog'; </span>// or unfoldingWord ???
    let taRepoBranch;
    try {
        taRepoBranch = checkingOptions?.taRepoBranch;
    } catch (trcBRerror) { }
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!taRepoBranch) taRepoBranch = 'master';
    let taRepoLanguageCode;
    try {
        taRepoLanguageCode = checkingOptions?.taRepoLanguageCode;
    } catch (trcLCerror) { }
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!taRepoLanguageCode) <span class="cstat-no" title="statement not covered" >taRepoLanguageCode = 'en';</span>
    let taRepoSectionName;
    try {
        taRepoSectionName = checkingOptions?.taRepoSectionName;
    } catch (trcSNerror) { }
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!taRepoSectionName) taRepoSectionName = 'translate';
    ...
    if (checkingOptions?.expectFullLink) {
        ...
        <span class="missing-if-branch" title="else path not taken" >E</span>if (!fieldText.startsWith('rc://*/'))
...
<span class="cstat-no" title="statement not covered" >            addNoticePartial({ priority: 889, message: `Unable to find/load TA article`, details: `linked from TN ${fieldName}`, excerpt: fieldText, location: `${ourLocation} ${filepath}` });</span>
        else <span class="missing-if-branch" title="if path not taken" >I</span>if (taFileContent.length &lt; 10)
<span class="cstat-no" title="statement not covered" >            addNoticePartial({ priority: 887, message: `TA article seems empty`, details: `linked from TN ${fieldName}`, excerpt: fieldText, location: `${ourLocation} ${filepath}` });</span>
    } catch (trcGCerror) {
