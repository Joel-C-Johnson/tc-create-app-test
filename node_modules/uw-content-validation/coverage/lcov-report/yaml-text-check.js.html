...
<head>
    <title>Code coverage report for yaml-text-check.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="index.html">All files</a> yaml-text-check.js</h1>
        <div class='clearfix'>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import yaml from 'yaml';
 ...
 */
export function <span class="fstat-no" title="function not covered" >checkYAMLText(</span>languageCode, repoCode, textName, YAMLText, givenLocation, checkingOptions) {
...
<span class="cstat-no" title="statement not covered" >    if (checkingOptions !== undefined) { //parameterAssert(typeof checkingOptions === 'object', `checkYAMLText: 'checkingOptions' parameter should be an object not a '${typeof checkingOptions}': ${JSON.stringify(checkingOptions)}`);</span>
    }
&nbsp;
...
&nbsp;
    const cytResult = <span class="cstat-no" title="statement not covered" >{ successList: [], noticeList: [] };</span>
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        cytResult.successList.push(successString);</span>
    }
    function <span class="fstat-no" title="function not covered" >addNotice(</span>noticeObject) {
...
<span class="cstat-no" title="statement not covered" >        if (noticeObject.characterIndex) { //parameterAssert(typeof noticeObject.characterIndex === 'number', `cManT addNotice: 'characterIndex' parameter should be a number not a '${typeof noticeObject.characterIndex}': ${noticeObject.characterIndex}`);</span>
        }
        // //parameterAssert(excerpt!==undefined, "cYt addNotice: 'excerpt' parameter should be defined");
<span class="cstat-no" title="statement not covered" >        if (noticeObject.excerpt) { //parameterAssert(typeof noticeObject.excerpt === 'string', `cManT addNotice: 'excerpt' parameter should be a string not a '${typeof noticeObject.excerpt}': ${noticeObject.excerpt}`);</span>
        }
...
<span class="cstat-no" title="statement not covered" >        if (noticeObject.debugChain) <span class="cstat-no" title="statement not covered" >noticeObject.debugChain = `checkYAMLText ${noticeObject.debugChain}`;</span></span>
<span class="cstat-no" title="statement not covered" >        cytResult.noticeList.push(noticeObject);</span>
    }
&nbsp;
    function <span class="fstat-no" title="function not covered" >ourCheckTextField(</span>lineNumber, fieldText, allowedLinks, optionalFieldLocation, checkingOptions) {
        /**
...
&nbsp;
        const resultObject = <span class="cstat-no" title="statement not covered" >checkTextField(languageCode, repoCode, 'YAML', '', fieldText, allowedLinks, optionalFieldLocation, checkingOptions);</span>
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        for (const noticeEntry of resultObject.noticeList)</span>
<span class="cstat-no" title="statement not covered" >            addNotice({ ...noticeEntry, lineNumber });</span>
    }
    // end of ourCheckTextField function
&nbsp;
    function <span class="fstat-no" title="function not covered" >checkYAMLLineContents(</span>lineNumber, lineText, lineLocation) {
&nbsp;
...
&nbsp;
        const allowedLinksInLine = <span class="cstat-no" title="statement not covered" >thisText.startsWith('url:') || thisText.startsWith('chapter_url:') || thisText.startsWith('rc:');</span>
<span class="cstat-no" title="statement not covered" >        if (thisText)</span>
<span class="cstat-no" title="statement not covered" >            ourCheckTextField(lineNumber, thisText, allowedLinksInLine, lineLocation, checkingOptions);</span>
    }
    // end of checkYAMLLine function
&nbsp;
&nbsp;
    function <span class="fstat-no" title="function not covered" >ourBasicFileChecks(</span>filename, fileText, fileLocation, checkingOptions) {
        // Does basic checks for small errors like leading/trailing spaces, etc.
&nbsp;
        // We assume that checking for compulsory fields is done elsewhere
&nbsp;
...
&nbsp;
        const resultObject = <span class="cstat-no" title="statement not covered" >checkTextfileContents(languageCode, repoCode, 'YAML', filename, fileText, fileLocation, checkingOptions);</span>
&nbsp;
...
<span class="cstat-no" title="statement not covered" >            addNotice(noticeEntry);</span>
        }
    }
...
&nbsp;
    ...
    const lines = <span class="cstat-no" title="statement not covered" >YAMLText.split('\n');</span>
    // debugLog(`  '${location}' has ${lines.length.toLocaleString()} total lines`);
    let formData;
<span class="cstat-no" title="statement not covered" >    try {</span>
<span class="cstat-no" title="statement not covered" >        formData = yaml.parse(YAMLText);</span>
        // debugLog("yaml.parse(YAMLText) got formData", JSON.stringify(formData));
    }
    catch (yamlError) {
        // console.error(`yaml parse error: ${yamlError.message}`);
<span class="cstat-no" title="statement not covered" >        addNotice({ priority: 920, message: yamlError.message, location: ourLocation })</span>
    }
    // Add the parsed YAML to our result
<span class="cstat-no" title="statement not covered" >    cytResult.formData = formData;</span>
&nbsp;
...
&nbsp;
<span class="cstat-no" title="statement not covered" >        checkYAMLLineContents(n, line, ourLocation);</span>
        // } else {
...
<span class="cstat-no" title="statement not covered" >    ourBasicFileChecks(textName, YAMLText, givenLocation, checkingOptions);</span>
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        cytResult.noticeList = removeDisabledNotices(cytResult.noticeList);</span>
    }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >    if (cytResult.noticeList.length)</span>
<span class="cstat-no" title="statement not covered" >        addSuccessMessage(`checkYAMLText v${YAML_VALIDATOR_VERSION_STRING} finished with ${cytResult.noticeList.length ? cytResult.noticeList.length.toLocaleString() : "zero"} notice${cytResult.noticeList.length === 1 ? '' : 's'}`);</span>
    else
<span class="cstat-no" title="statement not covered" >        addSuccessMessage(`No errors or warnings found by checkYAMLText v${YAML_VALIDATOR_VERSION_STRING}`)</span>
...
<span class="cstat-no" title="statement not covered" >    return cytResult;</span>
}
