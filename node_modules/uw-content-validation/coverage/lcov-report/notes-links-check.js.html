...
<head>
    <title>Code coverage report for notes-links-check.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="index.html">All files</a> notes-links-check.js</h1>
        <div class='clearfix'>
            
            <div class='fl pad1y space-right2'>
                <span class="strong">29.63% </span>
                <span class="quiet">Statements</span>
                <span class='fraction'>200/675</span>
            </div>
            ...
            <div class='fl pad1y space-right2'>
                ...
                <span class='fraction'>111/524</span>
            </div>
            ...
            <div class='fl pad1y space-right2'>
                <span class="strong">28.19% </span>
                <span class="quiet">Lines</span>
                <span class='fraction'>181/642</span>
            </div>
...
<a name='L1308'></a><a href='#L1308'>1308</a></td><td class="line-coverage quiet"><span class="cline-any cline-neutral">&nbsp;</span>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import localforage from 'localforage';
...
export async function <span class="fstat-no" title="function not covered" >clearCheckedArticleCache(</span>) {
<span class="cstat-no" title="statement not covered" >    userLog("clearCheckedArticleCache()…");</span>
<span class="cstat-no" title="statement not covered" >    await checkedArticleStore.clear();</span>
}
&nbsp;
/**
 *
 ...
 * @param {string} branch
 */
async function <span class="fstat-no" title="function not covered" >markAsChecked(</span>{ username, repository, path, branch }) {
    // debugLog(`markAsChecked(${username}, ${repository}, ${path}, ${branch})…`);
    const dummyPath = <span class="cstat-no" title="statement not covered" >Path.join(username, repository, branch, path);</span>
<span class="cstat-no" title="statement not covered" >    await checkedArticleStore.setItem(dummyPath, true);</span>
}
&nbsp;
/**
 *
 ...
 * @param {string} branch
 */
async function <span class="fstat-no" title="function not covered" >alreadyChecked(</span>{ username, repository, path, branch }) {
    // debugLog(`alreadyChecked(${username}, ${repository}, ${path}, ${branch})…`);
    const dummyPath = <span class="cstat-no" title="statement not covered" >Path.join(username, repository, branch, path);</span>
<span class="cstat-no" title="statement not covered" >    return await checkedArticleStore.getItem(dummyPath);</span>
}
&nbsp;
&nbsp;
/**
...
&nbsp;
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!fieldName.startsWith('TA ') &amp;&amp; !fieldName.startsWith('TW ') &amp;&amp; fieldName.indexOf('/') === -1) { //parameterAssert(fieldName === 'OccurrenceNote' || fieldName === 'Note' || fieldName === 'TWLink' || fieldName === 'Response' || fieldName === 'README' || fieldName === 'LICENSE', `checkNotesLinksToOutside: 'fieldName' parameter should be 'OccurrenceNote', 'Note', 'TWLink', 'Response', 'README' or 'LICENSE' not '${fieldName}'`);
    }
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (ourLocation &amp;&amp; ourLocation[0] !== ' ') <span class="cstat-no" title="statement not covered" >ourLocation = ` ${ourLocation}`;</span>
&nbsp;
...
&nbsp;
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (typeof excerptLength !== 'number' || <span class="branch-1 cbranch-no" title="branch not covered" >isNaN(excerptLength))</span> {
...
&nbsp;
    const getFile_ = (checkingOptions &amp;&amp; checkingOptions?.getFile) ? checkingOptions?.getFile : <span class="branch-1 cbranch-no" title="branch not covered" >cachedGetFile;</span>
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (languageCode === 'hbo' || languageCode === 'el-x-koine') <span class="cstat-no" title="statement not covered" >adjustedLanguageCode = 'en' </span>// This is a guess (and won't be needed for TWs when we switch to TWLs)
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!defaultLanguageCode) <span class="cstat-no" title="statement not covered" >defaultLanguageCode = adjustedLanguageCode;</span>
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!taRepoUsername) <span class="cstat-no" title="statement not covered" >taRepoUsername = defaultLanguageCode === 'en' ? 'unfoldingWord' : 'Door43-Catalog';</span>
    let taRepoBranch = checkingOptions?.taRepoBranch;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!taRepoBranch) taRepoBranch = DEFAULT_BRANCH;
    let twRepoUsername = checkingOptions?.twRepoUsername;
    if (!twRepoUsername) twRepoUsername = defaultLanguageCode === 'en' ? 'unfoldingWord' : <span class="branch-1 cbranch-no" title="branch not covered" >'Door43-Catalog';</span>
    let twRepoBranch = checkingOptions?.twRepoBranch;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!twRepoBranch) twRepoBranch = DEFAULT_BRANCH;
...
&nbsp;
    ...
    for (let i = 0; i &lt; givenV.length; i++) {
        <span class="missing-if-branch" title="else path not taken" >E</span>if (/^\d$/.test(givenV[i])) givenVfirstPart += givenV[i];
        else <span class="cstat-no" title="statement not covered" >break; </span>// stop at the first non-digit
    }
    let givenCint, givenVint;
    if (givenC &amp;&amp; givenV) {
        try {
            givenCint = (givenC === 'front') ? <span class="branch-0 cbranch-no" title="branch not covered" >0 </span>: ourParseInt(givenC);
            givenVint = (givenV === 'intro') ? <span class="branch-0 cbranch-no" title="branch not covered" >0 </span>: ourParseInt(givenVfirstPart);
            <span class="missing-if-branch" title="if path not taken" >I</span>if (givenVfirstPart !== givenV &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >givenV !== 'intro')</span> <span class="cstat-no" title="statement not covered" >debugLog(`From '${givenC}': '${givenV}' got '${givenC}': '${givenVfirstPart}' then integers ${givenCint}: ${givenVint} `);</span>
        } catch (cvError) {
<span class="cstat-no" title="statement not covered" >            console.error(`TN Link Check couldn’t parse given chapter and verse numbers for ${bookID} ${givenC}: ${givenV} ${fieldName} ' via ${givenC}:${givenVfirstPart} got ${givenCint}:${givenVint} with ${cvError}`);</span>
        }
    }
&nbsp;
    if (fieldName === 'x-tw' || fieldName === 'TWLink' || fieldName === 'SupportReference') {
        ...
        <span class="missing-if-branch" title="if path not taken" >I</span>if (trimStartFieldText !== fieldText) {
            const excerpt = <span class="cstat-no" title="statement not covered" >fieldText.substring(0, excerptLength).replace(/ /g, '␣') + (fieldText.length &gt; excerptLength ? '…' : '');</span>
<span class="cstat-no" title="statement not covered" >            addNoticePartial({ priority: 784, message: "Unexpected leading whitespace in link field", excerpt, characterIndex: 0, location: ourLocation });</span>
        }
        else <span class="missing-if-branch" title="if path not taken" >I</span>if (trimEndFieldText !== fieldText) {
            const excerpt = <span class="cstat-no" title="statement not covered" >(fieldText.length &gt; excerptLength ? '…' : '') + fieldText.substring(fieldText.length - 10).replace(/ /g, '␣');</span>
<span class="cstat-no" title="statement not covered" >            addNoticePartial({ priority: 785, message: "Unexpected trailing whitespace in link field", excerpt, characterIndex: fieldText.length - 1, location: ourLocation });</span>
        }
    }
&nbsp;
...
&nbsp;
    ...
    while ((regexResultArray = SIMPLE_IMAGE_REGEX.exec(fieldText))) {
        ...
        const [totalLink, altText, fetchLink] = <span class="cstat-no" title="statement not covered" >regexResultArray;</span>
        // if (altText !== 'OBS Image') userLog("This code was only checked for 'OBS Image' links");
<span class="cstat-no" title="statement not covered" >        if (!altText)</span>
<span class="cstat-no" title="statement not covered" >            addNoticePartial({ priority: 199, message: "Markdown image link has no alternative text", excerpt: totalLink, location: ourLocation });</span>
<span class="cstat-no" title="statement not covered" >        if (!fetchLink.startsWith('https://'))</span>
<span class="cstat-no" title="statement not covered" >            addNoticePartial({ priority: 749, message: "Markdown image link seems faulty", excerpt: fetchLink, location: ourLocation });</span>
        else <span class="cstat-no" title="statement not covered" >if (checkingOptions?.disableAllLinkFetchingFlag !== true) {</span>
            // debugLog(`Need to check existence of ${fetchLink}`);
<span class="cstat-no" title="statement not covered" >            try {</span>
                const responseData = <span class="cstat-no" title="statement not covered" >await cachedGetFileUsingFullURL({ uri: fetchLink });</span>
<span class="cstat-no" title="statement not covered" >                dataAssert(responseData.length &gt; 10, `Expected ${fetchLink} image file to be longer: ${responseData.length}`);</span>
                // debugLog("Markdown link fetch got response: ", responseData.length);
            } catch (flError) {
<span class="cstat-no" title="statement not covered" >                console.error(`Markdown image link fetch had an error fetching '${fetchLink}': ${flError}`);</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 748, message: "Error fetching markdown image link", excerpt: fetchLink, location: ourLocation });</span>
            }
        }
    }
    while ((regexResultArray = TITLED_IMAGE_REGEX.exec(fieldText))) {
        // debugLog(`Got markdown image in line ${lineNumber}:`, JSON.stringify(regexResultArray));
        const [totalLink, alt, fetchLink, title] = <span class="cstat-no" title="statement not covered" >regexResultArray;</span>
<span class="cstat-no" title="statement not covered" >        if (!alt)</span>
<span class="cstat-no" title="statement not covered" >            addNoticePartial({ priority: 199, message: "Markdown image link has no alternative text", excerpt: totalLink, location: ourLocation });</span>
<span class="cstat-no" title="statement not covered" >        if (!title)</span>
<span class="cstat-no" title="statement not covered" >            addNoticePartial({ priority: 348, message: "Markdown image link has no title text", excerpt: totalLink, location: ourLocation });</span>
<span class="cstat-no" title="statement not covered" >        if (!fetchLink.startsWith('https://'))</span>
<span class="cstat-no" title="statement not covered" >            addNoticePartial({ priority: 749, message: "Markdown image link seems faulty", excerpt: fetchLink, location: ourLocation });</span>
        else <span class="cstat-no" title="statement not covered" >if (checkingOptions?.disableAllLinkFetchingFlag !== true) {</span>
            // debugLog(`Need to check existence of ${fetchLink}`);
...
<span class="cstat-no" title="statement not covered" >        twLinkCount1 += 1;</span>
        //parameterAssert(regexResultArray.length === 4, `TW_INTERNAL_REGEX expected 4 fields (not ${regexResultArray.length})`);
        // eslint-disable-next-line no-unused-vars
        let [totalLink, _displayName, category, article] = <span class="cstat-no" title="statement not covered" >regexResultArray;</span>
<span class="cstat-no" title="statement not covered" >        processedLinkList.push(totalLink); </span>// Save the full link
&nbsp;
        const twRepoName = <span class="cstat-no" title="statement not covered" >`${defaultLanguageCode}_tw`;</span>
        // debugLog(`Got twRepoName=${twRepoName}`);
        const filepath = <span class="cstat-no" title="statement not covered" >`bible/${category}/${article.trim()}.md`;</span>
        // debugLog(`Got tW filepath=${filepath}`);
&nbsp;
<span class="cstat-no" title="statement not covered" >        if (!checkingOptions?.disableAllLinkFetchingFlag) {</span>
            // if (regexResultArray[3] === 'brother') debugLog(`Need to check ${fieldName} TW link ${regexResultArray} against ${twRepoName}`);
            const twPathParameters = <span class="cstat-no" title="statement not covered" >{ username: twRepoUsername, repository: twRepoName, path: filepath, branch: twRepoBranch };</span>
            let twFileContent;
<span class="cstat-no" title="statement not covered" >            try {</span>
<span class="cstat-no" title="statement not covered" >                twFileContent = await getFile_(twPathParameters);</span>
...
<span class="cstat-no" title="statement not covered" >                console.error(`checkNotesLinksToOutside(${bookID}, ${fieldName}, …) failed to load TW ${twRepoUsername} ${twRepoName}, ${filepath}, ${twRepoBranch}: ${trcGCerror.message}`);</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 882, message: `Error loading TW article`, details: `${twRepoUsername} ${twRepoName} ${twRepoBranch} ${filepath}`, excerpt: totalLink, location: `${ourLocation} ${filepath}: ${trcGCerror}` });</span>
            }
<span class="cstat-no" title="statement not covered" >            if (!twFileContent)</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 883, message: `Unable to find/load TW article`, details: `${twRepoUsername} ${twRepoName} ${twRepoBranch} ${filepath}`, excerpt: totalLink, location: `${ourLocation} ${filepath}` });</span>
            else { // we got the content of the TW article
<span class="cstat-no" title="statement not covered" >                if (twFileContent.length &lt; 10)</span>
<span class="cstat-no" title="statement not covered" >                    addNoticePartial({ priority: 881, message: `TW article seems empty`, details: `${twRepoUsername} ${twRepoName} ${twRepoBranch} ${filepath}`, excerpt: totalLink, location: `${ourLocation} ${filepath}` });</span>
...
<span class="cstat-no" title="statement not covered" >        taLinkCount1 += 1;</span>
        //parameterAssert(regexResultArray.length === 5, `TA_FULL_DISPLAY_LINK_REGEX expected 5 fields (not ${regexResultArray.length})`)
        // eslint-disable-next-line no-unused-vars
        let [totalLink, _displayName, foundLanguageCode, part, article] = <span class="cstat-no" title="statement not covered" >regexResultArray;</span>
<span class="cstat-no" title="statement not covered" >        processedLinkList.push(totalLink); </span>// Save the full link
&nbsp;
<span class="cstat-no" title="statement not covered" >        if (foundLanguageCode !== '*') {</span>
            const characterIndex = <span class="cstat-no" title="statement not covered" >TA_FULL_DISPLAY_LINK_REGEX.lastIndex - totalLink.length + 7;</span> // lastIndex points to the end of the field that was found
            const excerpt = <span class="cstat-no" title="statement not covered" >(characterIndex &gt; excerptHalfLength ? '…' : '') + fieldText.substring(characterIndex - excerptHalfLength, characterIndex + excerptHalfLengthPlus) + (characterIndex + excerptHalfLengthPlus &lt; fieldText.length ? '…' : '')</span>
<span class="cstat-no" title="statement not covered" >            addNoticePartial({ priority: 450, message: "Resource container link should have '*' language code", details: `not '${foundLanguageCode}'`, characterIndex, excerpt, location: ourLocation });</span>
        } else <span class="cstat-no" title="statement not covered" >if (repoCode === 'TN') { // but not TN2</span>
            // At the moment, tC can’t handle these links with * so we have to ensure that they're not there
            const characterIndex = <span class="cstat-no" title="statement not covered" >TA_FULL_DISPLAY_LINK_REGEX.lastIndex - totalLink.length + 7;</span> // lastIndex points to the end of the field that was found
            const excerpt = <span class="cstat-no" title="statement not covered" >(characterIndex &gt; excerptHalfLength ? '…' : '') + fieldText.substring(characterIndex - excerptHalfLength, characterIndex + excerptHalfLengthPlus) + (characterIndex + excerptHalfLengthPlus &lt; fieldText.length ? '…' : '')</span>
<span class="cstat-no" title="statement not covered" >            addNoticePartial({ priority: 950, message: "tC cannot yet process '*' language code", characterIndex, excerpt, location: ourLocation });</span>
        }
<span class="cstat-no" title="statement not covered" >        if (!foundLanguageCode || foundLanguageCode === '*') <span class="cstat-no" title="statement not covered" >foundLanguageCode = defaultLanguageCode;</span></span>
        const taRepoName = <span class="cstat-no" title="statement not covered" >`${foundLanguageCode}_ta`;</span>
        // debugLog(`Got taRepoName=${taRepoName}`);
        const filepath = <span class="cstat-no" title="statement not covered" >`${part}/${article}/01.md`;</span> // Other files are title.md, sub-title.md
        // debugLog(`Got tA filepath=${filepath}`);
&nbsp;
<span class="cstat-no" title="statement not covered" >        if (!checkingOptions?.disableAllLinkFetchingFlag) {</span>
            // functionLog(`checkNotesLinksToOutside: need to check against ${taRepoName}`);
            const taPathParameters = <span class="cstat-no" title="statement not covered" >{ username: taRepoUsername, repository: taRepoName, path: filepath, branch: taRepoBranch };</span>
            let taFileContent, alreadyGaveError = <span class="cstat-no" title="statement not covered" >false;</span>
<span class="cstat-no" title="statement not covered" >            try {</span>
<span class="cstat-no" title="statement not covered" >                taFileContent = await getFile_(taPathParameters);</span>
...
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 885, message: `Error loading TA article`, details: `${taRepoUsername} ${taRepoName} ${taRepoBranch} ${filepath}`, excerpt: totalLink, location: `${ourLocation} ${filepath}: ${trcGCerror}` });</span>
<span class="cstat-no" title="statement not covered" >                alreadyGaveError = true;</span>
            }
<span class="cstat-no" title="statement not covered" >            if (!alreadyGaveError) {</span>
<span class="cstat-no" title="statement not covered" >                if (!taFileContent)</span>
<span class="cstat-no" title="statement not covered" >                    addNoticePartial({ priority: 886, message: `Unable to find/load TA article`, details: `${taRepoUsername} ${taRepoName} ${taRepoBranch} ${filepath}`, excerpt: totalLink, location: `${ourLocation} ${filepath}` });</span>
                else <span class="cstat-no" title="statement not covered" >if (taFileContent.length &lt; 10)</span>
<span class="cstat-no" title="statement not covered" >                    addNoticePartial({ priority: 884, message: `TA article seems empty`, details: `${taRepoUsername} ${taRepoName} ${taRepoBranch} ${filepath}`, excerpt: totalLink, location: `${ourLocation} ${filepath}` });</span>
                else <span class="cstat-no" title="statement not covered" >if (checkingOptions?.disableLinkedTAArticlesCheckFlag !== true) {</span>
...
<span class="cstat-no" title="statement not covered" >                    if (await alreadyChecked(taPathParameters) !== true) {</span>
                        // functionLog(`checkNotesLinksToOutside needs to check TA article: ${filepath}`);
                        const checkTAFileResult = <span class="cstat-no" title="statement not covered" >await checkMarkdownText(foundLanguageCode, repoCode, `TA ${regexResultArray[3].trim()}.md`, taFileContent, ourLocation, checkingOptions);</span>
<span class="cstat-no" title="statement not covered" >                        for (const noticeObject of checkTAFileResult.noticeList)</span>
                            // Why don't we use addNoticePartial() here? (It adds bookID and fieldName.) Maybe it would be misleading???
<span class="cstat-no" title="statement not covered" >                            ctarResult.noticeList.push({ ...noticeObject, username: taRepoUsername, repoCode: 'TA', repoName: taRepoName, filename: filepath, location: ` linked to${ourLocation}`, extra: 'TA' });</span>
<span class="cstat-no" title="statement not covered" >                        ctarResult.checkedFileCount += 1;</span>
<span class="cstat-no" title="statement not covered" >                        ctarResult.checkedFilenames.push(`${regexResultArray[3].trim()}.md`);</span>
<span class="cstat-no" title="statement not covered" >                        ctarResult.checkedFilesizes = taFileContent.length;</span>
<span class="cstat-no" title="statement not covered" >                        ctarResult.checkedFilenameExtensions = ['md'];</span>
<span class="cstat-no" title="statement not covered" >                        ctarResult.checkedRepoNames.push(taRepoName);</span>
<span class="cstat-no" title="statement not covered" >                        markAsChecked(taPathParameters); </span>// don’t bother waiting for the result
                    }
                }
            }
        }
...
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (repoCode === 'TA' || fieldName.startsWith('TA ')) {
        // Check for relative TA links like [Borrow Words](../translate-transliterate/01.md)
<span class="cstat-no" title="statement not covered" >        while ((regexResultArray = TA_RELATIVE1_DISPLAY_LINK_REGEX.exec(fieldText))) {</span>
...
<span class="cstat-no" title="statement not covered" >            taLinkCount1 += 1;</span>
            ...
            // eslint-disable-next-line no-unused-vars
            let [totalLink, _displayName, article] = <span class="cstat-no" title="statement not covered" >regexResultArray;</span>
<span class="cstat-no" title="statement not covered" >            processedLinkList.push(totalLink); </span>// Save the full link
&nbsp;
            const taRepoName = <span class="cstat-no" title="statement not covered" >`${defaultLanguageCode}_ta`;</span>
            // debugLog(`Got taRepoName=${taRepoName}`);
            let TAsection = <span class="cstat-no" title="statement not covered" >'translate';</span>
<span class="cstat-no" title="statement not covered" >            if (fieldName.startsWith('checking/')) <span class="cstat-no" title="statement not covered" >TAsection = 'checking';</span></span>
            else <span class="cstat-no" title="statement not covered" >if (fieldName.startsWith('process/')) <span class="cstat-no" title="statement not covered" >TAsection = 'process';</span></span>
<span class="cstat-no" title="statement not covered" >            if (fieldName.startsWith('intro/')) <span class="cstat-no" title="statement not covered" >TAsection = 'intro';</span></span>
<span class="cstat-no" title="statement not covered" >            dataAssert(TAsection === 'translate' || TAsection === 'checking' || TAsection === 'process' || TAsection === 'intro', `Unexpected TA section name = '${TAsection}'`);</span>
            const filepath = <span class="cstat-no" title="statement not covered" >`${TAsection}/${article}/01.md`;</span> // Other files are title.md, sub-title.md
            // debugLog(`Got tA filepath=${filepath}`);
&nbsp;
<span class="cstat-no" title="statement not covered" >            if (!checkingOptions?.disableAllLinkFetchingFlag) {</span>
                // functionLog(`checkNotesLinksToOutside: need to check against ${taRepoName}`);
...
<span class="cstat-no" title="statement not covered" >        while ((regexResultArray = TA_RELATIVE2_DISPLAY_LINK_REGEX.exec(fieldText))) {</span>
...
<span class="cstat-no" title="statement not covered" >            taLinkCount1 += 1;</span>
            ...
            // eslint-disable-next-line no-unused-vars
            let [totalLink, _displayName, TAsection, article] = <span class="cstat-no" title="statement not covered" >regexResultArray;</span>
<span class="cstat-no" title="statement not covered" >            processedLinkList.push(totalLink); </span>// Save the full link
&nbsp;
            const taRepoName = <span class="cstat-no" title="statement not covered" >`${defaultLanguageCode}_ta`;</span>
            // debugLog(`Got taRepoName=${taRepoName}`);
...
&nbsp;
        if (foundLanguageCode !== '*') {
            ...
            const excerpt = (characterIndex &gt; excerptHalfLength ? '…' : <span class="branch-1 cbranch-no" title="branch not covered" >'')</span> + fieldText.substring(characterIndex - excerptHalfLength, characterIndex + excerptHalfLengthPlus) + (characterIndex + excerptHalfLengthPlus &lt; fieldText.length ? '…' : <span class="branch-1 cbranch-no" title="branch not covered" >'')</span>
        ...
        // debugLog(`Got tA filepath=${filepath}`);
&nbsp;
        <span class="missing-if-branch" title="else path not taken" >E</span>if (!checkingOptions?.disableAllLinkFetchingFlag) {
            // functionLog(`checkNotesLinksToOutside: need to check against ${taRepoName}`);
            ...
            if (!alreadyGaveError) {
                <span class="missing-if-branch" title="if path not taken" >I</span>if (!taFileContent)
<span class="cstat-no" title="statement not covered" >                    addNoticePartial({ priority: 886, message: `Unable to find/load TA article`, details: `${taRepoUsername} ${taRepoName} ${taRepoBranch} ${filepath}`, excerpt: totalLink, location: `${ourLocation} ${filepath}` });</span>
                else <span class="missing-if-branch" title="if path not taken" >I</span>if (taFileContent.length &lt; 10)
<span class="cstat-no" title="statement not covered" >                    addNoticePartial({ priority: 884, message: `TA article seems empty`, details: `${taRepoUsername} ${taRepoName} ${taRepoBranch} ${filepath}`, excerpt: totalLink, location: `${ourLocation} ${filepath}` });</span>
                else <span class="missing-if-branch" title="if path not taken" >I</span>if (checkingOptions?.disableLinkedTAArticlesCheckFlag !== true) {
                    // functionLog(`checkNotesLinksToOutside got ${checkingOptions?.disableLinkedTAArticlesCheckFlag} so checking TA article: ${filepath}`);
<span class="cstat-no" title="statement not covered" >                    if (await alreadyChecked(taPathParameters) !== true) {</span>
                        // functionLog(`checkNotesLinksToOutside needs to check TA article: ${filepath}`);
                        const checkTAFileResult = <span class="cstat-no" title="statement not covered" >await checkMarkdownText(foundLanguageCode, repoCode, `TA ${regexResultArray[3].trim()}.md`, taFileContent, ourLocation, checkingOptions);</span>
...
&nbsp;
        <span class="missing-if-branch" title="else path not taken" >E</span>if (!checkingOptions?.disableAllLinkFetchingFlag) {
            ...
            if (!twFileContent)
                ...
                <span class="missing-if-branch" title="if path not taken" >I</span>if (twFileContent.length &lt; 10)
<span class="cstat-no" title="statement not covered" >                    addNoticePartial({ priority: 881, message: `TW article seems empty`, details: `${twRepoUsername} ${twRepoName} ${twRepoBranch} ${filepath}`, excerpt: totalLink, location: `${ourLocation} ${filepath}` });</span>
                else <span class="missing-if-branch" title="if path not taken" >I</span>if (checkingOptions?.disableLinkedTWArticlesCheckFlag !== true) {
                    // functionLog(`checkNotesLinksToOutside got ${checkingOptions?.disableLinkedTWArticlesCheckFlag} so checking TW article: ${filepath}`);
<span class="cstat-no" title="statement not covered" >                    if (await alreadyChecked(twPathParameters) !== true) {</span>
                        // functionLog(`checkNotesLinksToOutside needs to check TW article: ${filepath}`);
                        const checkTWFileResult = <span class="cstat-no" title="statement not covered" >await checkMarkdownText(foundLanguageCode, repoCode, `TW ${regexResultArray[3].trim()}.md`, twFileContent, ourLocation, checkingOptions);</span>
<span class="cstat-no" title="statement not covered" >                        for (const noticeObject of checkTWFileResult.noticeList)</span>
                            // Why don't we use addNoticePartial() here? (It adds bookID and fieldName.) Maybe it would be misleading???
<span class="cstat-no" title="statement not covered" >                            ctarResult.noticeList.push({ ...noticeObject, username: twRepoUsername, repoCode: 'TW', repoName: twRepoName, filename: filepath, location: ` linked to${ourLocation}`, extra: 'TW' });</span>
<span class="cstat-no" title="statement not covered" >                        ctarResult.checkedFileCount += 1;</span>
<span class="cstat-no" title="statement not covered" >                        ctarResult.checkedFilenames.push(`${regexResultArray[3].trim()}.md`);</span>
<span class="cstat-no" title="statement not covered" >                        ctarResult.checkedFilesizes = twFileContent.length;</span>
<span class="cstat-no" title="statement not covered" >                        ctarResult.checkedFilenameExtensions = ['md'];</span>
<span class="cstat-no" title="statement not covered" >                        ctarResult.checkedRepoNames.push(twRepoName);</span>
<span class="cstat-no" title="statement not covered" >                        markAsChecked(twPathParameters); </span>// don’t bother waiting for the result
                    }
                }
                // else debugLog("checkNotesLinksToOutside: disableLinkedTWArticlesCheckFlag is set to TRUE!");
            }
...
<span class="cstat-no" title="statement not covered" >        otherBookBibleLinkCount1 += 1;</span>
        //parameterAssert(regexResultArray.length === 9, `BIBLE_FULL_HELP_REGEX expected 9 fields (not ${regexResultArray.length})`);
        let [totalLink, optionalN1, optionalB1, C1, V1, Lg, B2, C2, V2] = <span class="cstat-no" title="statement not covered" >regexResultArray;</span>
        // debugLog(`Lg='${Lg}' B2='${B2}' C2='${C2}' V2='${V2}'`);
<span class="cstat-no" title="statement not covered" >        processedLinkList.push(totalLink); </span>// Save the full link
&nbsp;
<span class="cstat-no" title="statement not covered" >        if (Lg !== '*' &amp;&amp; Lg !== languageCode)</span>
<span class="cstat-no" title="statement not covered" >            addNoticePartial({ priority: 669, message: "Unexpected language code in link", details: `resource language code is '${languageCode}'`, excerpt: Lg, location: ourLocation });</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >        if (optionalN1) { //parameterAssert(optionalB1, `Should have book name as well as number '${optionalN1}' in '${totalLink}'`);</span>
        }
<span class="cstat-no" title="statement not covered" >        if (optionalB1) {</span>
<span class="cstat-no" title="statement not covered" >            optionalB1 = `${optionalN1}${optionalB1}`.trim(); </span>// e.g., 1 Timothy
<span class="cstat-no" title="statement not covered" >            if (defaultLanguageCode === 'en') { // should be able to check the book name</span>
                const checkResult = <span class="cstat-no" title="statement not covered" >books.isGoodEnglishBookName(optionalB1);</span>
                // debugLog(optionalB1, "isGoodEnglishBookName checkResult", checkResult);
<span class="cstat-no" title="statement not covered" >                if (checkResult === undefined || checkResult === false)</span>
                    // NOTE: Our English bookname table has 'Song of Songs'
<span class="cstat-no" title="statement not covered" >                    addNoticePartial({ priority: optionalB1 === 'Song of Solomon' ? 43 : 143, message: `${optionalB1 === 'Song of Solomon' ? 'Unexpected' : 'Unknown'} Bible book name in TN RC link`, details: totalLink, excerpt: optionalB1, location: ourLocation });</span>
            }
        }
&nbsp;
        let linkBookCode = <span class="cstat-no" title="statement not covered" >B2 === '..' ? bookID : B2;</span>
&nbsp;
        const linkChapterInt = <span class="cstat-no" title="statement not covered" >ourParseInt(C2),</span> linkVerseInt = <span class="cstat-no" title="statement not covered" >ourParseInt(V2);</span>
<span class="cstat-no" title="statement not covered" >        try {</span>
<span class="cstat-no" title="statement not covered" >            if (ourParseInt(C1) !== linkChapterInt)</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 743, message: "Chapter numbers of markdown Bible link don’t match", details: `${C1} vs ${linkChapterInt}`, excerpt: totalLink, location: ourLocation });</span>
        } catch (ccError) {
<span class="cstat-no" title="statement not covered" >            console.error(`TN Link CheckA couldn’t compare chapter numbers for ${bookID} ${givenC}:${givenV} ${fieldName} with ${C1} from '${fieldText}': ${ccError}`);</span>
        }
<span class="cstat-no" title="statement not covered" >        try {</span>
<span class="cstat-no" title="statement not covered" >            if (ourParseInt(V1) !== linkVerseInt)</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 742, message: "Verse numbers of markdown Bible link don’t match", details: `${V1} vs ${linkVerseInt}`, excerpt: totalLink, location: ourLocation });</span>
        } catch (vvError) {
<span class="cstat-no" title="statement not covered" >            console.error(`TN Link CheckA couldn’t compare verse numbers for ${bookID} ${givenC}:${givenV} ${fieldName} with ${C1}:${V1} from '${fieldText}': ${vvError}`);</span>
        }
&nbsp;
<span class="cstat-no" title="statement not covered" >        if (linkBookCode === 'obs') {</span>
            const numStories = <span class="cstat-no" title="statement not covered" >50,</span> numFramesThisStory = <span class="cstat-no" title="statement not covered" >99;</span>
<span class="cstat-no" title="statement not covered" >            if (!linkChapterInt || linkChapterInt &lt; 1 || linkChapterInt &gt; numStories)</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 655, message: "Bad story number in markdown OBS help link", details: `${linkBookCode} ${linkChapterInt} vs ${numStories} chapters`, excerpt: totalLink, location: ourLocation });</span>
            else <span class="cstat-no" title="statement not covered" >if (!linkVerseInt || linkVerseInt &lt; 0 || linkVerseInt &gt; numFramesThisStory)</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 653, message: "Bad frame number in markdown OBS help link", details: `${linkBookCode} ${linkChapterInt}:${linkVerseInt} vs ${numFramesThisStory} verses`, excerpt: totalLink, location: ourLocation });</span>
        } else <span class="cstat-no" title="statement not covered" >if (linkBookCode.length) { // then we know which Bible book this link is to</span>
            // So we can check for valid C:V numbers
            let numChaptersThisBook, numVersesThisChapter;
<span class="cstat-no" title="statement not covered" >            logicAssert(linkBookCode.toLowerCase() !== 'obs', `BIBLE_FULL_HELP_REGEX linkBookCode shouldn’t be '${linkBookCode}' in notes-links-check`);</span>
<span class="cstat-no" title="statement not covered" >            try {</span>
<span class="cstat-no" title="statement not covered" >                numChaptersThisBook = books.chaptersInBook(linkBookCode);</span>
            } catch (tlcNCerror) {
<span class="cstat-no" title="statement not covered" >                debugLog(`checkNotesLinksToOutside1 with linkBookCode '${linkBookCode}' got error: ${tlcNCerror}`);</span>
<span class="cstat-no" title="statement not covered" >                numChaptersThisBook = 0;</span>
            }
<span class="cstat-no" title="statement not covered" >            try {</span>
<span class="cstat-no" title="statement not covered" >                numVersesThisChapter = books.versesInChapter(linkBookCode, linkChapterInt);</span>
            } catch (tlcNVerror) { }
<span class="cstat-no" title="statement not covered" >            if (!linkChapterInt || linkChapterInt &lt; 1 || linkChapterInt &gt; numChaptersThisBook)</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 655, message: "Bad chapter number in markdown Bible help link", details: `${linkBookCode} ${linkChapterInt} vs ${numChaptersThisBook} chapters`, excerpt: totalLink, location: ourLocation });</span>
            else <span class="cstat-no" title="statement not covered" >if (!linkVerseInt || linkVerseInt &lt; 0 || linkVerseInt &gt; numVersesThisChapter)</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 653, message: "Bad verse number in markdown Bible help link", details: `${linkBookCode} ${linkChapterInt}:${linkVerseInt} vs ${numVersesThisChapter} verses`, excerpt: totalLink, location: ourLocation });</span>
        } else
<span class="cstat-no" title="statement not covered" >            debugLog(`Seems BIBLE_FULL_HELP_REGEX '${totalLink}' didn't have a link book code!`);</span>
    }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        thisChapterBibleLinkCount1 += 1;</span>
        ...
        let [totalLink, optionalN1, optionalB1, C1, V1, V2] = <span class="cstat-no" title="statement not covered" >regexResultArray;</span>
<span class="cstat-no" title="statement not covered" >        processedLinkList.push(totalLink); </span>// Save the full link
&nbsp;
<span class="cstat-no" title="statement not covered" >        if (optionalN1) { //parameterAssert(optionalB1, `Should have book name as well as number '${optionalN1}'`);</span>
        }
<span class="cstat-no" title="statement not covered" >        if (optionalB1) {</span>
<span class="cstat-no" title="statement not covered" >            optionalB1 = `${optionalN1}${optionalB1}`.trim(); </span>// e.g., 1 Timothy
<span class="cstat-no" title="statement not covered" >            if (defaultLanguageCode === 'en') { // should be able to check the book name</span>
...
<span class="cstat-no" title="statement not covered" >                if (checkResult === undefined || checkResult === false)</span>
                    // NOTE: Our English bookname table has 'Song of Songs'
<span class="cstat-no" title="statement not covered" >                    addNoticePartial({ priority: optionalB1 === 'Song of Solomon' ? 43 : 143, message: `${optionalB1 === 'Song of Solomon' ? 'Unexpected' : 'Unknown'} Bible book name in Bible link`, details: totalLink, excerpt: optionalB1, location: ourLocation });</span>
            }
        }
&nbsp;
        let linkBookCode = <span class="cstat-no" title="statement not covered" >bookID;</span>
&nbsp;
        const linkVerseInt = <span class="cstat-no" title="statement not covered" >ourParseInt(V2);</span>
<span class="cstat-no" title="statement not covered" >        if (C1 === undefined) {</span>
<span class="cstat-no" title="statement not covered" >            if (linkBookCode.length === 0 || !books.isOneChapterBook(linkBookCode)) {</span>
...
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 555, message: "Possible missing chapter number in markdown Bible link", excerpt: totalLink, location: ourLocation });</span>
            }
<span class="cstat-no" title="statement not covered" >            C1 = '0'; </span>// Try to avoid consequential errors
        }
<span class="cstat-no" title="statement not covered" >        try {</span>
<span class="cstat-no" title="statement not covered" >            if (ourParseInt(C1) !== givenCint)</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 743, message: "Chapter numbers of markdown Bible link don’t match", details: `${C1} vs ${givenCint}`, excerpt: totalLink, location: ourLocation });</span>
        } catch (ccError) {
<span class="cstat-no" title="statement not covered" >            console.error(`TN Link Check1 couldn’t compare chapter numbers for ${bookID} ${givenC}:${givenV} ${fieldName} with ${C1} from '${fieldText}': ${ccError}`);</span>
        }
<span class="cstat-no" title="statement not covered" >        try {</span>
<span class="cstat-no" title="statement not covered" >            if (ourParseInt(V1) !== linkVerseInt)</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 742, message: "Verse numbers of markdown Bible link don’t match", details: `${V1} vs ${linkVerseInt}`, excerpt: totalLink, location: ourLocation });</span>
        } catch (vvError) {
<span class="cstat-no" title="statement not covered" >            console.error(`TN Link Check1 couldn’t compare verse numbers for ${bookID} ${givenC}:${givenV} ${fieldName} with ${C1}:${V1} from '${fieldText}': ${vvError}`);</span>
        }
&nbsp;
<span class="cstat-no" title="statement not covered" >        if (linkBookCode.length) { // then we know which Bible book this link is to</span>
            // So we can check for valid C:V numbers
            let numChaptersThisBook, numVersesThisChapter;
<span class="cstat-no" title="statement not covered" >            logicAssert(linkBookCode.toLowerCase() !== 'obs', `BIBLE_REGEX_THIS_CHAPTER_RELATIVE linkBookCode shouldn’t be '${linkBookCode}' in notes-links-check`);</span>
<span class="cstat-no" title="statement not covered" >            try {</span>
<span class="cstat-no" title="statement not covered" >                numChaptersThisBook = books.chaptersInBook(linkBookCode);</span>
            } catch (tlcNCerror) {
<span class="cstat-no" title="statement not covered" >                debugLog(`checkNotesLinksToOutside2 with linkBookCode '${linkBookCode}' got error: ${tlcNCerror}`);</span>
<span class="cstat-no" title="statement not covered" >                numChaptersThisBook = 0;</span>
            }
<span class="cstat-no" title="statement not covered" >            try {</span>
<span class="cstat-no" title="statement not covered" >                numVersesThisChapter = books.versesInChapter(linkBookCode, givenCint);</span>
            } catch (tlcNVerror) { }
<span class="cstat-no" title="statement not covered" >            if (!givenCint || givenCint &lt; 1 || givenCint &gt; numChaptersThisBook)</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 655, message: "Bad chapter number in markdown Bible link", details: `${linkBookCode} ${givenCint} vs ${numChaptersThisBook} chapters`, excerpt: totalLink, location: ourLocation });</span>
            else <span class="cstat-no" title="statement not covered" >if (!linkVerseInt || linkVerseInt &lt; 0 || linkVerseInt &gt; numVersesThisChapter)</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 653, message: "Bad verse number in markdown Bible link", details: `${linkBookCode} ${givenCint}:${linkVerseInt} vs ${numVersesThisChapter} verses`, excerpt: totalLink, location: ourLocation });</span>
        }
    }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        thisVerseBibleLinkCount1 += 1;</span>
        ...
        let [totalLink, V1, C2, V2] = <span class="cstat-no" title="statement not covered" >regexResultArray;</span>
<span class="cstat-no" title="statement not covered" >        processedLinkList.push(totalLink); </span>// Save the full link
&nbsp;
        let linkBookCode = <span class="cstat-no" title="statement not covered" >bookID;</span>
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        try {</span>
<span class="cstat-no" title="statement not covered" >            linkChapterInt = ourParseInt(C2);</span>
<span class="cstat-no" title="statement not covered" >            linkVerseInt = ourParseInt(V2);</span>
<span class="cstat-no" title="statement not covered" >            if (ourParseInt(V1) !== linkVerseInt)</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 742, message: "Verse numbers of markdown Bible link don’t match", details: `${V1} vs ${V2}`, excerpt: totalLink, location: ourLocation });</span>
        } catch (vvError) {
<span class="cstat-no" title="statement not covered" >            console.error(`TN Link Check1b couldn’t compare verse numbers for ${bookID} ${givenC}:${givenV} ${fieldName} ${V1} with ${C2}:${V2} from '${fieldText}': ${vvError}`);</span>
        }
&nbsp;
<span class="cstat-no" title="statement not covered" >        if (linkBookCode.length) { // then we know which Bible book this link is to</span>
            // So we can check for valid C:V numbers
            let numChaptersThisBook, numVersesThisChapter;
<span class="cstat-no" title="statement not covered" >            logicAssert(linkBookCode.toLowerCase() !== 'obs', `THIS_VERSE_TO_THIS_CHAPTER_BIBLE_REGEX linkBookCode shouldn’t be '${linkBookCode}' in notes-links-check`);</span>
<span class="cstat-no" title="statement not covered" >            try {</span>
<span class="cstat-no" title="statement not covered" >                numChaptersThisBook = books.chaptersInBook(linkBookCode);</span>
            } catch (tlcNCerror) {
<span class="cstat-no" title="statement not covered" >                debugLog(`checkNotesLinksToOutside3 with linkBookCode '${linkBookCode}' got error: ${tlcNCerror}`);</span>
<span class="cstat-no" title="statement not covered" >                numChaptersThisBook = 0;</span>
            }
<span class="cstat-no" title="statement not covered" >            try {</span>
<span class="cstat-no" title="statement not covered" >                numVersesThisChapter = books.versesInChapter(linkBookCode, linkChapterInt);</span>
...
<span class="cstat-no" title="statement not covered" >            if (!linkChapterInt || linkChapterInt &lt; 1 || linkChapterInt &gt; numChaptersThisBook)</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 655, message: "Bad chapter number in markdown Bible link", details: `${linkBookCode} ${linkChapterInt} vs ${numChaptersThisBook} chapters`, excerpt: totalLink, location: ourLocation });</span>
            else <span class="cstat-no" title="statement not covered" >if (!linkVerseInt || linkVerseInt &lt; 0 || linkVerseInt &gt; numVersesThisChapter)</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 653, message: "Bad verse number in markdown Bible link", details: `${linkBookCode} ${linkChapterInt}:${linkVerseInt} vs ${numVersesThisChapter} verses`, excerpt: totalLink, location: ourLocation });</span>
        }
    }
&nbsp;
    // Check for this-verse Bible links like [11](../03/11.md)
...
<span class="cstat-no" title="statement not covered" >        thisVerseBibleLinkCount1 += 1;</span>
        ...
        let [totalLink, V1a, V1b, C2, V2] = <span class="cstat-no" title="statement not covered" >regexResultArray;</span>
<span class="cstat-no" title="statement not covered" >        processedLinkList.push(totalLink); </span>// Save the full link
&nbsp;
        let linkBookCode = <span class="cstat-no" title="statement not covered" >bookID;</span>
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        try {</span>
<span class="cstat-no" title="statement not covered" >            verseInt1a = ourParseInt(V1a);</span>
<span class="cstat-no" title="statement not covered" >            verseInt1b = ourParseInt(V1b);</span>
<span class="cstat-no" title="statement not covered" >            linkChapterInt = ourParseInt(C2);</span>
<span class="cstat-no" title="statement not covered" >            linkVerseInt = ourParseInt(V2);</span>
<span class="cstat-no" title="statement not covered" >            if (verseInt1a !== linkVerseInt)</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 742, message: "Verse numbers of markdown Bible link don’t match", details: `${V1a} vs ${V2}`, excerpt: totalLink, location: ourLocation });</span>
        } catch (vvError) {
<span class="cstat-no" title="statement not covered" >            console.error(`TN Link Check1c couldn’t compare verse numbers for ${bookID} ${givenC}:${givenV} ${fieldName} ${V1a} with ${C2}:${V2} from '${fieldText}': ${vvError}`);</span>
        }
<span class="cstat-no" title="statement not covered" >        if (verseInt1b &lt;= verseInt1a)</span>
<span class="cstat-no" title="statement not covered" >            addNoticePartial({ priority: 741, message: "Verse numbers of markdown Bible link range out of order", details: `${V1a} to ${V1b}`, excerpt: totalLink, location: ourLocation });</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >        if (linkBookCode.length) { // then we know which Bible book this link is to</span>
            // So we can check for valid C:V numbers
            let numChaptersThisBook, numVersesThisChapter;
<span class="cstat-no" title="statement not covered" >            logicAssert(linkBookCode.toLowerCase() !== 'obs', `THIS_VERSE_RANGE_TO_THIS_CHAPTER_BIBLE_REGEX linkBookCode shouldn’t be '${linkBookCode}' in notes-links-check`);</span>
<span class="cstat-no" title="statement not covered" >            try {</span>
<span class="cstat-no" title="statement not covered" >                numChaptersThisBook = books.chaptersInBook(linkBookCode);</span>
            } catch (tlcNCerror) {
<span class="cstat-no" title="statement not covered" >                debugLog(`checkNotesLinksToOutside4 with linkBookCode '${linkBookCode}' got error: ${tlcNCerror}`);</span>
<span class="cstat-no" title="statement not covered" >                numChaptersThisBook = 0;</span>
            }
<span class="cstat-no" title="statement not covered" >            try {</span>
<span class="cstat-no" title="statement not covered" >                numVersesThisChapter = books.versesInChapter(linkBookCode, linkChapterInt);</span>
...
&nbsp;
    ...
    while ((regexResultArray = BIBLE_REGEX_THIS_BOOK_RELATIVE.exec(fieldText))) {
        ...
        processedLinkList.push(totalLink); // Save the full link
&nbsp;
        <span class="missing-if-branch" title="if path not taken" >I</span>if (optionalN1) { //parameterAssert(optionalB1, `Should have book name as well as number '${optionalN1}'`);
        }
        <span class="missing-if-branch" title="else path not taken" >E</span>if (optionalB1) {
            optionalB1 = `${optionalN1}${optionalB1}`.trim(); // e.g., 1 Timothy
            <span class="missing-if-branch" title="else path not taken" >E</span>if (defaultLanguageCode === 'en') { // should be able to check the book name
                const checkResult = books.isGoodEnglishBookName(optionalB1);
                // debugLog(optionalB1, "isGoodEnglishBookName checkResult", checkResult);
                <span class="missing-if-branch" title="if path not taken" >I</span>if (checkResult === undefined || checkResult === false)
                    // NOTE: Our English bookname table has 'Song of Songs'
<span class="cstat-no" title="statement not covered" >                    addNoticePartial({ priority: optionalB1 === 'Song of Solomon' ? 43 : 143, message: `${optionalB1==='Song of Solomon'?'Unexpected' : 'Unknown'} Bible book name in relative Bible link`, details: totalLink, excerpt: optionalB1, location: ourLocation });</span>
            }
        }
&nbsp;
        ...
        try {
            <span class="missing-if-branch" title="if path not taken" >I</span>if (ourParseInt(C1) !== linkChapterInt)
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 743, message: "Chapter numbers of markdown Bible link don’t match", details: `${C1} vs ${linkChapterInt}`, excerpt: totalLink, location: ourLocation });</span>
        } catch (ccError) {
<span class="cstat-no" title="statement not covered" >            console.error(`TN Link Check2 couldn’t compare chapter numbers for ${bookID} ${givenC}:${givenV} ${fieldName} with ${C2} from '${fieldText}': ${ccError}`);</span>
        }
        try {
            <span class="missing-if-branch" title="if path not taken" >I</span>if (ourParseInt(V1) !== linkVerseInt)
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 742, message: "Verse numbers of markdown Bible link don’t match", details: `${V1} vs ${linkVerseInt}`, excerpt: totalLink, location: ourLocation });</span>
        } catch (vvError) {
<span class="cstat-no" title="statement not covered" >            console.error(`TN Link Check2 couldn’t compare verse numbers for ${bookID} ${givenC}:${givenV} ${fieldName} with ${C2}:${V2} from '${fieldText}': ${vvError}`);</span>
        }
&nbsp;
        <span class="missing-if-branch" title="if path not taken" >I</span>if (linkBookCode.length) { // then we know which Bible book this link is to
            // So we can check for valid C:V numbers
            let numChaptersThisBook, numVersesThisChapter;
<span class="cstat-no" title="statement not covered" >            logicAssert(linkBookCode.toLowerCase() !== 'obs', `BIBLE_REGEX_THIS_BOOK_RELATIVE linkBookCode shouldn’t be '${linkBookCode}' in notes-links-check`);</span>
<span class="cstat-no" title="statement not covered" >            try {</span>
<span class="cstat-no" title="statement not covered" >                numChaptersThisBook = books.chaptersInBook(linkBookCode);</span>
            } catch (tlcNCerror) {
<span class="cstat-no" title="statement not covered" >                debugLog(`checkNotesLinksToOutside5 with linkBookCode '${linkBookCode}' got error: ${tlcNCerror}`);</span>
<span class="cstat-no" title="statement not covered" >                numChaptersThisBook = 0;</span>
            }
<span class="cstat-no" title="statement not covered" >            try {</span>
<span class="cstat-no" title="statement not covered" >                numVersesThisChapter = books.versesInChapter(linkBookCode, linkChapterInt);</span>
...
<span class="cstat-no" title="statement not covered" >        thisBookBibleLinkCount1 += 1;</span>
        //parameterAssert(regexResultArray.length === 9, `BCV_V_TO_THIS_BOOK_BIBLE_REGEX expected 9 fields (not ${regexResultArray.length})`);
        let [totalLink, optionalN1, optionalB1, C1, V1a, V1b, B2, C2, V2] = <span class="cstat-no" title="statement not covered" >regexResultArray;</span>
<span class="cstat-no" title="statement not covered" >        processedLinkList.push(totalLink); </span>// Save the full link
&nbsp;
<span class="cstat-no" title="statement not covered" >        if (optionalN1) { //parameterAssert(optionalB1, `Should have book name as well as number '${optionalN1}'`);</span>
        }
...
<span class="cstat-no" title="statement not covered" >            console.error(`TN Link Check2b couldn’t compare chapter numbers for ${bookID} ${givenC}:${givenV} ${fieldName} with ${C2} from '${fieldText}': ${ccError}`);</span>
        }
<span class="cstat-no" title="statement not covered" >        try {</span>
<span class="cstat-no" title="statement not covered" >            if (ourParseInt(V1a) !== linkVerseInt)</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 742, message: "Verse numbers of markdown Bible link don’t match", details: `${V1a} vs ${linkVerseInt}`, excerpt: totalLink, location: ourLocation });</span>
        } catch (vvError) {
<span class="cstat-no" title="statement not covered" >            console.error(`TN Link Check2b couldn’t compare verse numbers for ${bookID} ${givenC}:${givenV} ${fieldName} with ${C2}:${V2} from '${fieldText}': ${vvError}`);</span>
        }
<span class="cstat-no" title="statement not covered" >        try {</span>
<span class="cstat-no" title="statement not covered" >            if (ourParseInt(V1b) &lt;= ourParseInt(V1a))</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 741, message: "Verse numbers of markdown Bible link range out of order", details: `${V1a} to ${V1b}`, excerpt: totalLink, location: ourLocation });</span>
        } catch (vvError) {
<span class="cstat-no" title="statement not covered" >            console.error(`TN Link Check2c couldn’t compare verse numbers for ${bookID} ${givenC}:${givenV} ${fieldName} with ${C2}:${V2} from '${fieldText}': ${vvError}`);</span>
        }
&nbsp;
<span class="cstat-no" title="statement not covered" >        if (linkBookCode.length) { // then we know which Bible book this link is to</span>
            // So we can check for valid C:V numbers
            let numChaptersThisBook, numVersesThisChapter;
<span class="cstat-no" title="statement not covered" >            logicAssert(linkBookCode.toLowerCase() !== 'obs', `BCV_V_TO_THIS_BOOK_BIBLE_REGEX linkBookCode shouldn’t be '${linkBookCode}' in notes-links-check`);</span>
<span class="cstat-no" title="statement not covered" >            try {</span>
<span class="cstat-no" title="statement not covered" >                numChaptersThisBook = books.chaptersInBook(linkBookCode);</span>
            } catch (tlcNCerror) {
<span class="cstat-no" title="statement not covered" >                debugLog(`checkNotesLinksToOutside6 with linkBookCode '${linkBookCode}' got error: ${tlcNCerror}`);</span>
<span class="cstat-no" title="statement not covered" >                numChaptersThisBook = 0;</span>
            }
<span class="cstat-no" title="statement not covered" >            try {</span>
<span class="cstat-no" title="statement not covered" >                numVersesThisChapter = books.versesInChapter(linkBookCode, linkChapterInt);</span>
...
<span class="cstat-no" title="statement not covered" >        thisChapterBibleLinkCount1 += 1;</span>
        ...
        let [totalLink, optionalN1, optionalB1, C1, V1a, V1b, V2] = <span class="cstat-no" title="statement not covered" >regexResultArray;</span>
<span class="cstat-no" title="statement not covered" >        processedLinkList.push(totalLink); </span>// Save the full link
&nbsp;
<span class="cstat-no" title="statement not covered" >        if (optionalN1) {</span>
...
<span class="cstat-no" title="statement not covered" >            console.error(`TN Link Check2d couldn’t compare verse numbers for ${bookID} ${givenC}:${givenV} ${fieldName} with ${C1}:${V2} from '${fieldText}': ${vvError}`);</span>
        }
<span class="cstat-no" title="statement not covered" >        try {</span>
<span class="cstat-no" title="statement not covered" >            if (ourParseInt(V1b) &lt;= ourParseInt(V1a))</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 741, message: "Verse numbers of markdown Bible link range out of order", details: `${V1a} to ${V1b}`, excerpt: totalLink, location: ourLocation });</span>
        } catch (vvError) {
<span class="cstat-no" title="statement not covered" >            console.error(`TN Link Check2e couldn’t compare verse numbers for ${bookID} ${givenC}:${givenV} ${fieldName} with ${C1}:${V2} from '${fieldText}': ${vvError}`);</span>
        }
&nbsp;
<span class="cstat-no" title="statement not covered" >        if (linkBookCode.length) { // then we know which Bible book this link is to</span>
            // So we can check for valid C:V numbers
            let numVersesThisChapter;
<span class="cstat-no" title="statement not covered" >            logicAssert(linkBookCode.toLowerCase() !== 'obs', `BCV_V_TO_THIS_CHAPTER_BIBLE_REGEX linkBookCode shouldn’t be '${linkBookCode}' in notes-links-check`);</span>
            // try {
            //     numChaptersThisBook = books.chaptersInBook(linkBookCode);
            // } catch (tlcNCerror) { }
<span class="cstat-no" title="statement not covered" >            try {</span>
<span class="cstat-no" title="statement not covered" >                numVersesThisChapter = books.versesInChapter(linkBookCode, givenC);</span>
            } catch (tlcNVerror) { }
<span class="cstat-no" title="statement not covered" >            if (!linkVerseInt || linkVerseInt &lt; 0 || linkVerseInt &gt; numVersesThisChapter)</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 653, message: "Bad verse number in markdown Bible link", details: `${linkBookCode} ${givenC}:${linkVerseInt} vs ${numVersesThisChapter} verses`, excerpt: totalLink, location: ourLocation });</span>
        }
    }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        otherBookBibleLinkCount1 += 1;</span>
        ...
        let [totalLink, optionalN1, optionalB1, C1, V1, B2, C2, V2] = <span class="cstat-no" title="statement not covered" >regexResultArray;</span>
<span class="cstat-no" title="statement not covered" >        processedLinkList.push(totalLink); </span>// Save the full link
&nbsp;
<span class="cstat-no" title="statement not covered" >        if (optionalN1) {</span>
            //parameterAssert(optionalB1, `Should have book name as well as number '${optionalN1}'`);
...
<span class="cstat-no" title="statement not covered" >            console.error(`TN Link Check3 couldn’t compare chapter numbers for ${bookID} ${givenC}:${givenV} ${fieldName} with ${C1} from '${fieldText}': ${ccError}`);</span>
        }
<span class="cstat-no" title="statement not covered" >        try {</span>
<span class="cstat-no" title="statement not covered" >            if (ourParseInt(V1) !== linkVerseInt)</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 742, message: "Verse numbers of markdown Bible link don’t match", details: `${V1} vs ${linkVerseInt}`, excerpt: totalLink, location: ourLocation });</span>
        } catch (vvError) {
<span class="cstat-no" title="statement not covered" >            console.error(`TN Link Check3 couldn’t compare verse numbers for ${bookID} ${givenC}:${givenV} ${fieldName} with ${C1}:${V1} from '${fieldText}': ${vvError}`);</span>
        }
&nbsp;
<span class="cstat-no" title="statement not covered" >        if (linkBookCode.length) { // then we know which Bible book this link is to</span>
            // So we can check for valid C:V numbers
            let numChaptersThisBook, numVersesThisChapter;
<span class="cstat-no" title="statement not covered" >            logicAssert(linkBookCode.toLowerCase() !== 'obs', `BIBLE_REGEX_OTHER_BOOK_ABSOLUTE linkBookCode shouldn’t be '${linkBookCode}' in notes-links-check`);</span>
<span class="cstat-no" title="statement not covered" >            try {</span>
<span class="cstat-no" title="statement not covered" >                numChaptersThisBook = books.chaptersInBook(linkBookCode);</span>
            } catch (tlcNCerror) {
<span class="cstat-no" title="statement not covered" >                debugLog(`checkNotesLinksToOutside8 with linkBookCode '${linkBookCode}' got error: ${tlcNCerror}`);</span>
<span class="cstat-no" title="statement not covered" >                numChaptersThisBook = 0;</span>
            }
<span class="cstat-no" title="statement not covered" >            try {</span>
<span class="cstat-no" title="statement not covered" >                numVersesThisChapter = books.versesInChapter(linkBookCode, linkChapterInt);</span>
...
<span class="cstat-no" title="statement not covered" >        if (linkBookCode.length) { // then we know which Bible book this link is to</span>
            // So we can check for valid C:V numbers
            let numChaptersThisBook, numVersesThisChapter;
<span class="cstat-no" title="statement not covered" >            logicAssert(linkBookCode.toLowerCase() !== 'obs', `BIBLE_REGEX_OTHER_BOOK_RELATIVE linkBookCode shouldn’t be '${linkBookCode}' in notes-links-check`);</span>
<span class="cstat-no" title="statement not covered" >            try {</span>
<span class="cstat-no" title="statement not covered" >                numChaptersThisBook = books.chaptersInBook(linkBookCode);</span>
            } catch (tlcNCerror) {
<span class="cstat-no" title="statement not covered" >                debugLog(`checkNotesLinksToOutside9 with linkBookCode '${linkBookCode}' got error: ${tlcNCerror}`);</span>
<span class="cstat-no" title="statement not covered" >                numChaptersThisBook = 0;</span>
            }
<span class="cstat-no" title="statement not covered" >            try {</span>
<span class="cstat-no" title="statement not covered" >                numVersesThisChapter = books.versesInChapter(linkBookCode, linkChapterInt);</span>
...
&nbsp;
        ...
        if (optionalB1) {
            ...
            <span class="missing-if-branch" title="else path not taken" >E</span>if (defaultLanguageCode === 'en') { // should be able to check the book name
                ...
                // debugLog(optionalB1, "isGoodEnglishBookName checkResult", checkResult);
                <span class="missing-if-branch" title="if path not taken" >I</span>if (checkResult === undefined || checkResult === false)
<span class="cstat-no" title="statement not covered" >                    addNoticePartial({ priority: 144, message: "Unknown Bible book name in TN link", details: totalLink, excerpt: optionalB1, location: ourLocation });</span>
            }
        }
&nbsp;
        let linkBookCode = B2 === '..' ? bookID : <span class="branch-1 cbranch-no" title="branch not covered" >B2;</span>
&nbsp;
        const linkChapterInt = ourParseInt(C2), linkVerseInt = ourParseInt(V2);
        try {
            <span class="missing-if-branch" title="if path not taken" >I</span>if (ourParseInt(C1) !== linkChapterInt)
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 743, message: "Chapter numbers of markdown TN link don’t match", details: `${C1} vs ${linkChapterInt}`, excerpt: totalLink, location: ourLocation });</span>
        } catch (ccError) {
<span class="cstat-no" title="statement not covered" >            console.error(`TN Link Check3b couldn’t compare chapter numbers for ${bookID} ${givenC}:${givenV} ${fieldName} with ${C1} from '${fieldText}': ${ccError}`);</span>
        }
        try {
            <span class="missing-if-branch" title="if path not taken" >I</span>if (ourParseInt(V1) !== linkVerseInt)
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 752, message: "Verse numbers of markdown TN link don’t match", details: `${V1} vs ${linkVerseInt}`, excerpt: totalLink, location: ourLocation });</span>
        } catch (vvError) {
<span class="cstat-no" title="statement not covered" >            console.error(`TN Link Check3b couldn’t compare verse numbers for ${bookID} ${givenC}:${givenV} ${fieldName} with ${C1}:${V1} from '${fieldText}': ${vvError}`);</span>
        }
&nbsp;
        <span class="missing-if-branch" title="if path not taken" >I</span>if (linkBookCode.length) { // then we know which Bible book this link is to
            // So we can check for valid C:V numbers
            let numChaptersThisBook, numVersesThisChapter;
<span class="cstat-no" title="statement not covered" >            logicAssert(linkBookCode.toLowerCase() !== 'obs', `TN_REGEX linkBookCode shouldn’t be '${linkBookCode}' in notes-links-check`);</span>
<span class="cstat-no" title="statement not covered" >            try {</span>
<span class="cstat-no" title="statement not covered" >                numChaptersThisBook = books.chaptersInBook(linkBookCode);</span>
            } catch (tlcNCerror) {
<span class="cstat-no" title="statement not covered" >                debugLog(`checkNotesLinksToOutside10 with linkBookCode '${linkBookCode}' got error: ${tlcNCerror}`);</span>
<span class="cstat-no" title="statement not covered" >                numChaptersThisBook = 0;</span>
            }
<span class="cstat-no" title="statement not covered" >            try {</span>
<span class="cstat-no" title="statement not covered" >                numVersesThisChapter = books.versesInChapter(linkBookCode, linkChapterInt);</span>
...
<span class="cstat-no" title="statement not covered" >            if (!linkChapterInt || linkChapterInt &lt; 1 || linkChapterInt &gt; numChaptersThisBook)</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 656, message: "Bad chapter number in markdown TN link", details: `${linkBookCode} ${linkChapterInt} vs ${numChaptersThisBook} chapters`, excerpt: totalLink, location: ourLocation });</span>
            else <span class="cstat-no" title="statement not covered" >if (!linkVerseInt || linkVerseInt &lt; 0 || linkVerseInt &gt; numVersesThisChapter)</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 654, message: "Bad verse number in markdown TN link", details: `${linkBookCode} ${linkChapterInt}:${linkVerseInt} vs ${numVersesThisChapter} verses`, excerpt: totalLink, location: ourLocation });</span>
        }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        generalLinkCount1 += 1;</span>
        //parameterAssert(regexResultArray.length === 3, `SIMPLE_DISPLAY_LINK_REGEX expected 3 fields (not ${regexResultArray.length})`);
        // eslint-disable-next-line no-unused-vars
        let [totalLink, displayText, uri] = <span class="cstat-no" title="statement not covered" >regexResultArray;</span>
<span class="cstat-no" title="statement not covered" >        processedLinkList.push(totalLink); </span>// Save the full link
&nbsp;
<span class="cstat-no" title="statement not covered" >        if (!checkingOptions?.disableAllLinkFetchingFlag) {</span>
            const dummyPathParameters = <span class="cstat-no" title="statement not covered" >{ username: uri, repository: '', path: '', branch: '' };</span>
<span class="cstat-no" title="statement not covered" >            if (await alreadyChecked(dummyPathParameters) !== true) {</span>
                // debugLog(`checkNotesLinksToOutside general link check needs to check: ${uri}`);
                const serverString = <span class="cstat-no" title="statement not covered" >uri.replace('://', '!!!').split('/')[0].replace('!!!', '://').toLowerCase();</span> // Get the bit before any forward slashes
&nbsp;
                // NOTE: These message strings must match RenderProcessedResults.js
<span class="cstat-no" title="statement not covered" >                if (!serverString.endsWith('door43.org') &amp;&amp; !serverString.endsWith('unfoldingword.org') &amp;&amp; !serverString.endsWith('ufw.io')) // don't try to fetch general links</span>
<span class="cstat-no" title="statement not covered" >                    addNoticePartial({ priority: 32, message: `Untested general/outside link`, details: "please manually double-check link—probably no problem", excerpt: totalLink, location: ourLocation });</span>
                else { // Try to fetch general links
                    let generalFileContent, hadError = <span class="cstat-no" title="statement not covered" >false;</span>
<span class="cstat-no" title="statement not covered" >                    try {</span>
                        ...
                        const response = <span class="cstat-no" title="statement not covered" >await fetch(uri);</span>
<span class="cstat-no" title="statement not covered" >                        if (response.ok) {// if HTTP-status is 200-299</span>
<span class="cstat-no" title="statement not covered" >                            generalFileContent = await response.text();</span>
                            // debugLog(`General link ${displayText} @ ${uri} got: (${generalFileContent.length}) ${generalFileContent.substring(0, 10)}...`);
                        } else <span class="cstat-no" title="statement not covered" >throw new Error(`Our Network error: ${response.statusCode}`);</span>
                    } catch (trcGCerror) {
...
<span class="cstat-no" title="statement not covered" >                        addNoticePartial({ priority: 82, message: `Error loading general link`, details: "please double-check link—there may be no problem", excerpt: totalLink, location: `${ourLocation}: ${trcGCerror}` });</span>
<span class="cstat-no" title="statement not covered" >                        hadError = true;</span>
                    }
<span class="cstat-no" title="statement not covered" >                    if (!hadError &amp;&amp; !generalFileContent)</span>
<span class="cstat-no" title="statement not covered" >                        addNoticePartial({ priority: 783, message: `Unable to find/load general link`, excerpt: totalLink, location: ourLocation });</span>
                    else <span class="cstat-no" title="statement not covered" >if (generalFileContent) { // we got the content of the general article</span>
<span class="cstat-no" title="statement not covered" >                        if (generalFileContent.length &lt; 10)</span>
<span class="cstat-no" title="statement not covered" >                            addNoticePartial({ priority: 781, message: `Linked general article seems empty`, excerpt: totalLink, location: ourLocation });</span>
                    }
                }
<span class="cstat-no" title="statement not covered" >                await markAsChecked(dummyPathParameters); </span>// don’t bother waiting for the result of this async call
            }
            // else debugLog(`Had already checked '${displayText}' ${uri}`);
&nbsp;
<span class="cstat-no" title="statement not covered" >            if (uri.startsWith('http:'))</span>
<span class="cstat-no" title="statement not covered" >                addNoticePartial({ priority: 152, message: "Should http link be https", excerpt: totalLink, location: ourLocation });</span>
        }
    }
&nbsp;
    ...
    if (totalLinks1 &gt; linkCount1) {
        ...
        addNoticePartial({ priority: 648, message: "Unusual [ ]( ) link(s)—not a recognized Bible or TA, TN, or TW link", details: `need to carefully check ${leftoverLinksList1.length === 1 ? '"' + leftoverLinksList1[0] + '"' : <span class="branch-1 cbranch-no" title="branch not covered" >JSON.stringify(leftoverLinksList1)}</span>`, location: ourLocation });
    }
    ...
    if (totalLinks2 &gt; linkCount2) {
        ...
        addNoticePartial({ priority: 649, message: "Unusual [[ ]] link(s)—not a recognized TA or TW link", details: `need to carefully check ${leftoverLinksList2.length === 1 ? '"' + leftoverLinksList2[0] + '"' : <span class="branch-1 cbranch-no" title="branch not covered" >JSON.stringify(leftoverLinksList2)}</span>`, location: ourLocation });
    }
&nbsp;
    ...
    if (leftCount !== rightCount)
        ...
        <span class="missing-if-branch" title="if path not taken" >I</span>if (leftCount !== rightCount)
<span class="cstat-no" title="statement not covered" >            addNoticePartial({ priority: 844, message: `Mismatched [[rc:// ]] link characters`, details: `left=${leftCount.toLocaleString()}, right=${rightCount.toLocaleString()}`, location: ourLocation });</span>
    }
    // Check for badly formed [ ]( ) links
...
&nbsp;</pre></td></tr></table></pre>
            ...
            <div class='footer quiet pad2 space-top1 center small'>
                ...
                at Wed Jul 21 2021 08:06:19 GMT+1200 (New Zealand Standard Time)
            </div>
        </div>
