...
<head>
    <title>Code coverage report for usfm-js-check.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="index.html">All files</a> usfm-js-check.js</h1>
        <div class='clearfix'>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import { toJSON } from 'usfm-js';
...
&nbsp;
&nbsp;
export function <span class="fstat-no" title="function not covered" >checkUSFMToJSON(</span>bookID, filename, givenText, givenLocation, checkingOptions) {
    /*
...
<span class="cstat-no" title="statement not covered" >        result.successList.push(successString);</span>
    ...
    function <span class="fstat-no" title="function not covered" >addNotice6to7(</span>{ priority, message, lineNumber, characterIndex, excerpt, location }) {
        /**
...
<span class="cstat-no" title="statement not covered" >        if (characterIndex) { //parameterAssert(typeof characterIndex === 'number', `cUSFMjs addNotice6to7: 'characterIndex' parameter should be a number not a '${typeof characterIndex}': ${characterIndex}`);</span>
        }
        // //parameterAssert(excerpt !== undefined, "cUSFMjs addNotice6to7: 'excerpt' parameter should be defined");
<span class="cstat-no" title="statement not covered" >        if (excerpt) { //parameterAssert(typeof excerpt === 'string', `cUSFMjs addNotice6to7: 'excerpt' parameter should be a string not a '${typeof excerpt}': ${excerpt}`);</span>
        }
...
<span class="cstat-no" title="statement not covered" >        result.noticeList.push({ priority, message, bookID, lineNumber, characterIndex, excerpt, location });</span>
    }
&nbsp;
&nbsp;
    // Set your options here -- change values or comment out lines
    const convertOptions = <span class="cstat-no" title="statement not covered" >{</span>
    ...
    const convertCheckResult = <span class="cstat-no" title="statement not covered" >runUsfmJsCheck(givenText, convertOptions);</span>
    // NOTE: We haven’t figured out how to get ERRORS out of this parser yet
&nbsp;
<span class="cstat-no" title="statement not covered" >    if (!convertCheckResult.isValidUSFM)</span>
<span class="cstat-no" title="statement not covered" >        addNotice6to7({ priority: 943, message: `USFM3 toJSON Check doesn’t pass`, location: ourLocation });</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    addSuccessMessage(`Checked USFM-JS`);</span>
